...
def binary_to_hexa(s):
    mp = {"0000" : '0',"0001" : '1',"0010" : '2',"0011" : '3',"0100" : '4',"0101" : '5',"0110" : '6',
...
def encrypt(pt, rkb, rk):
    ...
    for i in range(0, 6):
        ...
        sbox_str = permute(sbox_str, permutation, 32)
...
ii="".join(hex(ord(j)-ord('d'))[2:] for j in ii).upper()
for key in open("allkeys.txt"):
    key="01101110010111100111101100001001001110001100111111010001"
    # Splitting
    ...
    for i in range(0, 6):
        ...
        rk.append(binary_to_hexa(round_key))
    print(c,end='\r')
    cipher_text=binary_to_hexa(encrypt(ii,rkb,rk))
    
    rkb_rev = rkb[::-1]
    rk_rev = rk[::-1]
    cipher_text = encrypt(ii, rkb_rev, rk_rev)
    print(cipher_text)
    cipher_text=binary_to_hexa(cipher_text)
    
    cipher_text="".join(chr(ord('d')+int(j,16)) for j in cipher_text)
    print(cipher_text)
    break